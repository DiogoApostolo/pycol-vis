...
class ImageComplexity:
    ...
    def visualize_specific_images(self, image_list, measure="entropy", figsize=(15, 3)):
        ...
        Parameters:
        - image_paths (list): a List with the image paths corresponding to the images to visualize
        - measure (str): The name of the intrinsic measure to visualize, should correspond to the name stored in the self.images Df
        - figsize (tuple): The figure size of each image to be displayed
        '''
        ...
        for i, path in enumerate(image_list):
            ...
            img = self.load_image(path, convert_rgb=True)
            axes[i].imshow(img)
            axes[i].axis("off")

        

            if(measure is not None and measure in self.images.columns):
                
                row = self.images[self.images["image_path"] == path]
                print(row)
